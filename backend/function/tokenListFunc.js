...
module.exports = function () {
  ...
  this.getPriceETHV2 = async function (address) {
    ...
    const uniswapV2FactoryABI = JSON.parse(
      fs.readFileSync(
        path.resolve(__dirname, "../abi/uniswapV2FactoryABI.json")
      )
    );
    let uniswapV2FactoryAddress = "0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f";
    let uniswapV2Factory = new web3.eth.Contract(
      uniswapV2FactoryABI,
      uniswapV2FactoryAddress
    );

    let pool = await uniswapV2Factory.methods
      .getPair(address, addressWETH)
      .call();
    const tokenABI = JSON.parse(
      fs.readFileSync(path.resolve(__dirname, "../abi/tokenABI.json"))
    );
  ...
  this.getPriceETHV3 = async function (address) {
    const uniswapV3FactoryABI = JSON.parse(
      fs.readFileSync(
        path.resolve(__dirname, "../abi/uniswapV3FactoryABI.json")
      )
    );
    const uniswapV3PoolABI = JSON.parse(
      fs.readFileSync(path.resolve(__dirname, "../abi/uniswapV3PoolABI.json"))
    );
    ...
    let uniswapV3Factory = new web3.eth.Contract(
      uniswapV3FactoryABI,
      uniswapV3FactoryAddress
    );

    let addressWETH = "0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2";
    let pool = await uniswapV3Factory.methods
      .getPool(address, addressWETH, 3000)
      .call();
    ...
    let price =
      Math.pow(sqrtPriceX96, 2) /
      Math.pow(2, 192) /
      Math.pow(10, 18 - tokenDecimal);
